...
image_folder = '/media/wenuka/New Volume-G/01.FYP/Dataset/Train Set Screenshots'  # Replace 'your_image_folder_path' with the path to your image folder
txt_file = 'train_32_dataset.txt'  # Replace 'your_text_file.txt' with the path to your text file
first_T_dir = '/media/wenuka/New Volume-G/01.FYP/Tool_1/Training/Attention_maps_train/1st Tier'
both_T_dir = '/media/wenuka/New Volume-G/01.FYP/Tool_1/Training/Attention_maps_train/Both Tiers'
save_heatmaps_dir = '/media/wenuka/New Volume-G/01.FYP/Dataset/New_overlapped_32_no_red/train'
opacity = 0.55
    ...
    for line in file:
        if len(line.strip())>0:
            ...
            if os.path.exists(image_path):
                ...
                slice_name = f'{filename[:-4]}_slice{value}.png'
                ...
                first_T_avg = os.path.join(first_T_dir,'AVG',filename[:-4]+".png")
                ...
                both_T_avg = os.path.join(both_T_dir,"AVG",filename[:-4]+".png")
                ...
                cv2.imwrite(os.path.join(save_heatmaps_dir,'1st Tier','All',filename[:-4]+".png"),blended_first_T_all)
                cv2.imwrite(os.path.join(save_heatmaps_dir,'1st Tier','AVG',filename[:-4]+".png"),blended_first_T_avg)
                cv2.imwrite(os.path.join(save_heatmaps_dir,'Both Tiers','All',filename[:-4]+".png"),blended_both_T_all)
                cv2.imwrite(os.path.join(save_heatmaps_dir,'Both Tiers','AVG',filename[:-4]+".png"),blended_both_T_avg)
            else:
