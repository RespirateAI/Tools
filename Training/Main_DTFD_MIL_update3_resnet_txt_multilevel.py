...
from LungData_from_txt import LungDataset, custom_collate
...
def main():
    ...
    log_dir = os.path.join(params.log_dir, "new2", "logresnet_comb_t5.txt")
    save_dir = os.path.join(params.log_dir, "new2", "random_binary_resnet_comb_t5.pth")
    z = vars(params).copy()
...
def test_attention_DTFD_preFeat_MultipleMean(
    ...
    data = LungDataset(
        "/media/ravindu/SSD-PLU3/Lung_CT_Dataset/Lung-PET-CT-Dx/Resnet EMB/Resnet_32_avg/combined",
        "/media/ravindu/SSD-PLU3/Lung_CT_Dataset/Lung-PET-CT-Dx/Resnet EMB/Resnet_32_avg/Training/part_1.txt",
    )
...
def train_attention_preFeature_DTFD(
    ...
    data = LungDataset(
        "/media/ravindu/SSD-PLU3/Lung_CT_Dataset/Lung-PET-CT-Dx/Resnet EMB/Resnet_32_avg/combined",
        "/media/ravindu/SSD-PLU3/Lung_CT_Dataset/Lung-PET-CT-Dx/Resnet EMB/Resnet_32_avg/Training/train_5.txt",
    )
    dataloader = DataLoader(data, params.batch_size, collate_fn=custom_collate)
    ...
    for batch in dataloader:
        ...
        for tidx, tslide in enumerate(slide_names_batch):
            ...
            current_features = features_batch[tidx]
            index_chunk_dic = {}
            tfeat_tensors = []

            tfeat_tensor = torch.tensor(current_features[1]).to(params.device)
            tfeat_tensors.append(tfeat_tensor)
            ...
            tfeat_tensor = torch.tensor(current_features[2]).to(params.device)
            tfeat_tensors.append(tfeat_tensor)
            feat_index = list(range(tfeat_tensor.shape[0]))
            random.shuffle(feat_index)
            index_chunk_list = np.array_split(np.array(feat_index), 2)
            ...
            tfeat_tensor = torch.tensor(current_features[3]).to(params.device)
            tfeat_tensors.append(tfeat_tensor)
            feat_index = list(range(tfeat_tensor.shape[0]))
            random.shuffle(feat_index)
            index_chunk_list = np.array_split(np.array(feat_index), 1)
            ...
            tfeat_tensor = torch.tensor(current_features[4]).to(params.device)
            tfeat_tensors.append(tfeat_tensor)
            feat_index = list(range(tfeat_tensor.shape[0]))
            random.shuffle(feat_index)
            index_chunk_list = np.array_split(np.array(feat_index), 1)
            ...
            for i in range(1, 5):
                ...
                for tindex in index_chunk_list:
                    ...
                    max_inst_feat = subFeat_tensor.index_select(
