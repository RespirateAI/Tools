...
loaded_numpy_data_dict = np.load(
    "./Attention/new/attention_weights_resnet_4096_test.npy", allow_pickle=True
).item()
...
for key in keys:
    ...
    for i in range(reshaped_attention.shape[0]):
        ...
        plt.savefig(
            f"AM/Attention Maps 4/Test_set/Resnet/Sag/1st Tier/All/{name}_slice{i}.png"
        )
        # plt.savefig(f"Attention Maps 4/Val_set/Resnet/1st Tier/AVG/{name}.png")
